# Expanded Features Extraction Script
import os
import librosa
import numpy as np
import pandas as pd
...
# Configuration
ROOT_DIR = r"D:\new researches\Car research2\dataset\DB3_segmented_audio"  # Root directory
OUTPUT_FEATURES_CSV = os.path.join(ROOT_DIR, "DB3_Categorized_features4.csv")  # Path for saving extracted features
...
def extract_features(file_path):
    ...
    - Mel-Spectrogram statistics (mean, variance)
    - Additional temporal patterns (onset times, durations)
    """
    try:
        # Load the audio file
        ...
        mel_variance = np.var(mel_spectrogram_db)
...
if all_features:
    ...
    for f in all_features:
        flat = {
            ...
            "hnr_mean": f["hnr_mean"],
            "mel_mean": f["mel_mean"],
            "mel_variance": f["mel_variance"],
            "onset_strength_mean": f["onset_strength_mean"],
